...
Install the [Bazel build system](https://bazel.build), clone this repository, and run:

```sh
bazel build //src:ASTRID -c opt
```

Once the build is complete, the executable will be located at `bazel-bin/src/ASTRID`.
