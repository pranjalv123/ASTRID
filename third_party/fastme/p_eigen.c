...
{
	int low, hi, i, j, k, it, istate;
	double tiny, t;

	istate = 0;
	tiny = sqrt (pow ((double) BASE, (double) (1 - DIGITS)));
...
complex compl_ (double re, double im)
{
	complex r;
...
int cmatinv (complex *x, int n, int m, double *space)
{
	...
	for (i=0; i<n; i++)
	{
		...
		if (xmaxsize < ee)
		{
			Warning ( (char*)"Det goes to zero at %8d!", i+1);
			return (-1);
		}
		if (irow[i] != i)
		{
	...
	*low = l;
	...
	do
	{
		for (done=1, i=l; i<=k; i++)
		{
			...
			if ( (fabs (c) > DBL_EPSILON) && ( fabs (r) > DBL_EPSILON) )
			{
				g = r / BASE;
				f = 1;
				s = c + r;
...
/* Reduce the submatrix in rows and columns low through hi of real matrix
 * mat to Hessenberg form by elementary similarity transformations */
		...
		{	/* look for single small subdiagonal element */
			...
			if (l == en - 1)		/* two roots found */
			{
				...
				else 				/* real pair */
				{
					z = (p < 0) ? p - z : p + z;
					valr[en-1] = x + z;
					valr[en] = (fabs(z) < DBL_EPSILON) ? x + z : x - w / z;
					if (job)
					{
						x = mat[pos (en, en-1, n)];
						s = fabs (x) + fabs (z);
						...
						{
							z = mat[pos (en - 1, j, n)];
							mat[pos (en - 1, j, n)] = q * z + p * mat[pos (en, j, n)];
	...
	if ( fabs (norm) > DBL_EPSILON )
	{
		...
		for (en=n-1; en>=0; en--)
		{
			...
			if ((q = vali[en]) < 0)		/* complex vector */
			{
				...
				{
					v = cdiv (compl_ (0.0, -mat[pos (en - 1, en, n)]),
							compl_ (mat[pos (en - 1, en - 1, n)] - p, q));
				...
				for (i=en-2; i>=0; i--)
				{
					...
					{
						m = i;
						if ( fabs (vali[i]) < DBL_EPSILON )
						{
							v = cdiv (compl_ (-ra, -sa), compl_ (w, q));
						...
						else	/* solve complex equations */
						{
							...
							v.im = (valr[i] - p) * 2 * q;
							if ( (fabs (v.re) + fabs (v.im)) > DBL_EPSILON )
								v.re = eps * norm * (fabs(w) + fabs (q) +
									fabs (x) + fabs (y) + fabs (z));

							v = cdiv (compl_ (x * r - z * ra + q * sa, x * s - z * sa - q * ra), v);
							mat[pos (i, en - 1, n)] = v.re;
							mat[pos (i, en, n)] = v.im;
							if (fabs (x) > fabs (z) + fabs (q))
							{
								mat[pos (i + 1, en - 1, n)] = (-ra - w * mat[pos (i, en - 1, n)] +
									q * mat[pos (i, en, n)]) / x;
							...
							{
								v = cdiv (compl_ (-r - y * mat[pos (i, en - 1, n)],
									-s - y * mat[pos (i, en, n)]), compl_ (z, q));
			...
			else if ( fabs (q) < DBL_EPSILON )	/* real vector */
			{
				m = en;
				mat[pos (en, en, n)] = 1;
				for (i=en-1; i>=0; i--)
...
int ludcmp (double **a, int n, double *d)
{
	...
	*d = 1.0;
	for (i=0; i<n; i++)
	{
		...
		if (big == 0.0)
			Exit ( (char*)"Singular matrix in routine LUDCMP.");
