...
static unsigned long mt[N_];	/* the array for the state vector */
static int mti=N_+1;			/* mti==N_+1 means mt[N_] is not initialized */
...
double uniformGenerator ()
{
	...
	{
		if (mti == N_+1)		/* if sgenrand() has not been called, */
			sgenrand(4357);	/* a default initial seed is used */

		for (kk=0; kk<N_-M_; kk++)
		{
			y = (mt[kk]&UPPER_MASK)|(mt[kk+1]&LOWER_MASK);
			mt[kk] = mt[kk+M_] ^ (y >> 1) ^ mag01[y & 0x1];
		}
		for (; kk<N_-1; kk++)
		{
			y = (mt[kk]&UPPER_MASK)|(mt[kk+1]&LOWER_MASK);
			mt[kk] = mt[kk+(M_-N_)] ^ (y >> 1) ^ mag01[y & 0x1];
		}

		y = (mt[N_-1]&UPPER_MASK)|(mt[0]&LOWER_MASK);
		mt[N_-1] = mt[M_-1] ^ (y >> 1) ^ mag01[y & 0x1];
